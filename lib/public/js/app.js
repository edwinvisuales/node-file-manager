...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    var setCurFiles = function (relPath) {
      $http.get('api' + relPath)
        ...
        .error(function (data, status) {
          if(data === '¡La ruta no existe!') {
            document.location.href="/";
            M.toast({html: `Error: ${status} ${data}`});
            console.log(data);
          }
        });
    };
    ...
    $scope.icon = function(e){
      if(
        e.name.substr(name.length -3) == 'png' ||
        e.name.substr(name.length -4) == 'jpeg'||
        e.name.substr(name.length -3) == 'jpg' ||
        e.name.substr(name.length -3) == 'gif'
        )
        {
          return 'photo_size_select_actual';
        }else if(
          e.name.substr(name.length -3) == 'mp4' ||
          e.name.substr(name.length -3) == 'mov'
          )
          {
            return 'local_movies';
          }
      ...
      // file.folder ? 'folder' : 'photo_size_select_actual'
    }
    ...
    var downloadFile = function (file) {
      window.location.href = 'api' + file.relPath;
    };
    ...
    $scope.showPhoto = function(file) {
      $scope.nombre = file.name;
      $scope.ruta = file.relPath;
      $scope.archivo = file;
      $(document).ready(function(){
        $('#modal-showPhoto').modal('open');
      })
    };
    $scope.check = function(file){
      // file.folder ?  FM.clickFile(file) : showPhoto(e)
      if(file.folder) {
        return FM.clickFile(file);
      }
      else {
        return $scope.showPhoto(file);
      }
    }
    ...
    FM.upload = function () {
      ...
      for (let i=0;i< FM.uploadFile.length; i++) {
          var formData = new FormData();
          formData.append('upload', FM.uploadFile[i]);
          url.push('api' + FM.curFolderPath + encodeURI(FM.uploadFile[i].name));
        httpRequest('POST', url[i], {type: 'UPLOAD_FILE'}, formData, {
